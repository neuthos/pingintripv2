...
import {
  bookingConditionsSections,
  bookingConditionsMeta,
} from "@/data/booking-conditions";
import {FileText, ArrowRight, ChevronRight} from "lucide-react";
import {useState, useEffect} from "react";

export default function BookingConditionsPage() {
  const t = useTranslations("BookingConditionsPage");
  const [activeSection, setActiveSection] = useState("");

  useEffect(() => {
    const handleScroll = () => {
      const sections = bookingConditionsSections.map((s) => ({
        id: s.id,
  ...
  const formattedDate = new Date(
    bookingConditionsMeta.lastUpdated
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-10 lg:gap-14">
          ...
          <aside className="hidden lg:block">
            <nav className="sticky top-24">
              ...
              <ul className="space-y-1">
                {bookingConditionsSections.map((section) => (
                  <li key={section.id}>
                    <a
                      href={`#${section.id}`}
          ...
          <article className="min-w-0">
            ...
            <div className="space-y-12">
              {bookingConditionsSections.map((section, index) => (
                <section
                  key={section.id}
                  id={section.id}
                ...
                >
                  ...
                  {index < bookingConditionsSections.length - 1 && (
